...
#define GPIO_SPI_SCK_PIN (GPIO3)

#define GPIO_SPI_MISO_PORT (GPIOB)
#define GPIO_SPI_MISO_PIN (GPIO4)

#define GPIO_SPI_MOSI_PORT (GPIOB)
#define GPIO_SPI_MOSI_PIN (GPIO5)

#define GPIO_SPI_CS_PORT (GPIOB)
#define GPIO_SPI_CS_PIN (GPIO10)
...
static spi_setup(void)
{
  ...
  gpio_set_af(GPIO_SPI_SCK_PORT, GPIO_AF6, GPIO_SPI_SCK_PIN);

  gpio_mode_setup(GPIO_SPI_MOSI_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_SPI_MOSI_PIN);
  gpio_set_af(GPIO_SPI_MOSI_PORT, GPIO_AF6, GPIO_SPI_MOSI_PIN);

  gpio_mode_setup(GPIO_SPI_MISO_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_SPI_MISO_PIN);
  gpio_set_af(GPIO_SPI_MISO_PORT, GPIO_AF6, GPIO_SPI_MISO_PIN);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    mcp2515_reset();
    delay(300);
    uint8_t data = mcp2515_read_byte(CANSTAT);
    ...
    // spi_send8(SPI3, 0);
    // // spi_write(SPI3, 0x00);
    // uint8_t data = spi_read8(SPI3);
    // usart_send_blocking(USART2, data);

    // #if defined(TX)
    //     mcp2515_write_buffer(tx_data, 8);
    //     delay(1000000);
    //     // #elif defined(RX)
    //     mcp2515_read_buffer(rx_data);
    //     for (int i = 0; i < 8; i++)
    //     {
    //       usart_send_blocking(USART2, rx_data[i]);
    //     }
    //     usart_send_blocking(USART2, 0xFE);
    // #else
    // #errer
    // #endif
    //     spi_set_nss_high(SPI3);
