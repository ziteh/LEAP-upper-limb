...
void max485_send(uint8_t *data, uint32_t length)
{
  ...
  /* MAX485 enable Tx, disable Rx. */
  HAL_GPIO_WritePin(MAX485_DE_RE_GPIO_PORT, MAX485_DE_RE_GPIO_PIN, GPIO_PIN_SET);

  /* Send packet. */
  if (HAL_UART_Transmit(&MAX485_UART, data, length, HAL_MAX_DELAY) != HAL_OK)
  {
    while (1) /* Died. */
      ;
  }
  ...
  /* MAX485 enable Rx, disable Tx. */
  HAL_GPIO_WritePin(MAX485_DE_RE_GPIO_PORT, MAX485_DE_RE_GPIO_PIN, GPIO_PIN_RESET);
#else
#error
#endif
}
