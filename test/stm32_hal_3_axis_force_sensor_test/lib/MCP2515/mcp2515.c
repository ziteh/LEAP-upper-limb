...
void setRegister(u8 reg, const uint8_t value)
{
    ...
    SPI_SELECT();
    HAL_SPI_TransmitReceive(&hspi2, buffer, buffer, 3, SPI_TIMEOUT);
    SPI_DESELECT();
}
...
void setRegisters(u8 reg, u8 *value, const u8 n)
{
    ...
    *(buffer + 1) = reg;
    ...
    SPI_SELECT();
    HAL_SPI_TransmitReceive(&hspi2, buffer, buffer, n + 2, SPI_TIMEOUT);
    SPI_DESELECT();
    if (buffer != NULL)
    {
        free(buffer);
...
void modifyRegister(const u8 reg, const u8 mask, const u8 data)
{
    ...
    SPI_SELECT();
    HAL_SPI_TransmitReceive(&hspi2, buffer, buffer, 4, SPI_TIMEOUT);
    SPI_DESELECT();
}
...
u8 getStatus()
{
    ...
    SPI_SELECT();
    HAL_SPI_TransmitReceive(&hspi2, buffer, buffer, 2, SPI_TIMEOUT);
    SPI_DESELECT();
    return buffer[1];
}
