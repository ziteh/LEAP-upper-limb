...
#define MCP2515_SPI (SPI2)
#define MCP2515_SPI_CS_PORT (GPIOB)
#define MCP2515_SPI_CS_PIN (GPIO6)
// extern SPI_HandleTypeDef hspi1;

// XXX
void SPI_SELECT(void)
{
    gpio_clear(MCP2515_SPI_CS_PORT, MCP2515_SPI_CS_PIN);
}
...
void SPI_DESELECT(void)
{
    ...
    while (!(SPI_SR(MCP2515_SPI) & SPI_SR_TXE))
    { /* None. */
    }
    while (SPI_SR(MCP2515_SPI) & SPI_SR_BSY)
    { /* None. */
    }

    gpio_set(MCP2515_SPI_CS_PORT, MCP2515_SPI_CS_PIN);
}
...
void setRegister(u8 reg, const uint8_t value)
{
    ...
    spi_send8(MCP2515_SPI, buffer[0]);
    spi_send8(MCP2515_SPI, buffer[1]);
    spi_send8(MCP2515_SPI, buffer[2]);
    SPI_DESELECT();
}
...
void setRegisters(u8 reg, u8 *value, const u8 n)
{
    ...
    *(buffer + 1) = reg;
    ...
    for (int i = 0; i < n + 2; i++)
    {
        spi_send8(MCP2515_SPI, buffer[i]);
    }
    SPI_DESELECT();
    if (buffer != NULL)
    {
...
u8 readRegister(const u8 reg)
{
    ...
    spi_send8(MCP2515_SPI, buffer[1]);
    spi_send8(MCP2515_SPI, buffer[2]);
    buffer[2] = spi_read8(MCP2515_SPI);
    SPI_DESELECT();
    return buffer[2];
}
...
void readRegisters(const u8 reg, u8 *value, const u8 n)
{
    ...
    *(buffer + 1) = reg;
    ...
    for (int i = 0; i < n; i++)
    {
        spi_send8(MCP2515_SPI, 0);
        buffer[i + 2] = spi_read8(MCP2515_SPI);
    }
    SPI_DESELECT();
...
void modifyRegister(const u8 reg, const u8 mask, const u8 data)
{
    ...
    spi_send8(MCP2515_SPI, buffer[1]);
    spi_send8(MCP2515_SPI, buffer[2]);
    spi_send8(MCP2515_SPI, buffer[3]);
    SPI_DESELECT();
}
...
u8 getStatus()
{
    ...
    spi_send8(MCP2515_SPI, buffer[0]);
    spi_send8(MCP2515_SPI, buffer[1]);
    buffer[1] = spi_read8(MCP2515_SPI);
    SPI_DESELECT();
    return buffer[1];
}
